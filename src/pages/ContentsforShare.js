...
const ContentsforShare = (props) => {
  const [DBresult1, setDBResult1] = useState('로딩중');
  useEffect(() => {
    console.log('props.ResultList[동이름]: ', props.ResultList[0]);
    console.log('props.id: ', props.id);
    API.get('/api/result/popup/', {
      params: { dong_name: props.ResultList[0] },
    }).then((res) => {
      setDBResult1(res.data);
      console.log('DBresult: ', res.data);
    });
  }, []);
      ...
      <div className="d1">
        ...
        <div className="result1">
          ...
          <span className="bold1">
            {props.ResultList[props.ResultList.length - 1]}
          </span>
          {'  '}
          {Math.round(props.ResultList[props.ResultList.length - 2])}분
          <br />
        </div>
      </div>
...
export default ContentsforShare;
